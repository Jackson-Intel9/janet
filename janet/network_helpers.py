...
def get_network(act_func_name, dim_in, dim_out, dense_sizes):
    ...
    _net_init, net_pred = stax.serial(*seq)

    def net_init(key):
        return _net_init(key, (-1, dim_in))[1]

    return net_init, net_pred


def write_trained_network_to_h5(
    outname, net_params, xmin, xmax, ymin, ymax, activation, loss_history, **metadata
):
    ...
    """
    if not HAS_H5PY:
        raise ImportError("Must have h5py installed to use this function")
    ...
    with h5py.File(outname, "w") as hdf:
        ...
        hdf[s] = np.zeros(1)
        hdf["loss_history"] = np.atleast_1d(loss_history)
        for key, value in metadata.items():
            hdf["metadata/" + key] = value
...
def train_fixed_loss_data(
    ...
    pars = get_pars(state)
    loss_min = opt_func(pars, opt_data)
